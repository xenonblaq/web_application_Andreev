...
Далее будет представлено полное описание принципов модели.
...
$score_i = 0.25 \cdot risk_i + 0.15 \cdot \left(w_{ret_i} \cdot ret + w_{liq_i} \cdot liq + w_{vol_i} \cdot vol\right) + 0.6 \cdot news$
...
Модель имеет параметр "вероятность скачка", обозначающий вероятность изменения новостного фона на противоположный.
 ...
 Если текущий новостной фон позитивный, то есть, $news_{bg} \geq 0.5$, то:

$$news_t = random(0.4; 1)$$

 Если же текущий новостной фон негативный, то есть, $news_{bg} < 0.5$, то
